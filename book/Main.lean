...
def config : Config where
  ...
  sourceLink := some "https://github.com/subfish-zhou/tacticbook"
  issueLink := some "https://github.com/subfish-zhou/tacticbook/issues"
  linkTargets := fun st => st.localTargets ++ st.remoteTargets

def main := manualMain (%doc Tacticbook) (config := config)
