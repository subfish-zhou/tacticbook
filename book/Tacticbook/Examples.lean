...
initialize stripDashDashExt : EnvExtension Bool ←
  registerEnvExtension (pure false)

private def stripDashDashCommentLines (hl : Highlighted) : Highlighted :=
  let kept := hl.lines.filter fun line =>
    let trimmedLeft := line.toString.dropWhile Char.isWhitespace
    !trimmedLeft.startsWith "--"
  kept.foldl (init := .empty) (· ++ ·)
...
def lean : CodeBlockExpander
  | args, code => do
    ...
    let stripDashDash := stripDashDashExt.getState (← getEnv)
    ...
    let pre : Option Highlighted := if stripDashDash then pre.map stripDashDashCommentLines else pre
    ...
    for item in mid do
      ...
      let code := if stripDashDash then stripDashDashCommentLines code else code
    ...
    let post : Option Highlighted := if stripDashDash then post.map stripDashDashCommentLines else post
...
private partial def firstTokenText? (stx : Syntax) : Option String :=
  match stx with
  | .atom _ s => some s
  | .ident _ s _ _ => some s.toString
  | .node _ _ args =>
